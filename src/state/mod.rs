...
use crate::{msg::bindings::BabylonMsg, utils::babylon_epoch_chain::NUM_BTC_TXS, bindings::msg_btc_finalized_header};
use cosmwasm_std::{StdError, Storage};
use babylon_proto::babylon::{zoneconcierge::v1::BtcTimestamp, btccheckpoint::v1::TransactionInfo};
...
pub fn handle_btc_timestamp(storage: &mut dyn Storage, btc_ts: &BtcTimestamp) -> Result<Option<BabylonMsg>, StdError> {
    // extract and init/handle BTC headers
    let btc_headers = &btc_ts.btc_headers;
    if btc_light_client::is_initialized(storage) {
        btc_light_client::handle_btc_headers_from_babylon(storage, btc_headers)
    ...
    let (epoch, raw_ckpt, proof_epoch_sealed, txs_info) = babylon_epoch_chain::extract_data_from_btc_ts(btc_ts)?;
    if babylon_epoch_chain::is_initialized(storage) {
        babylon_epoch_chain::handle_epoch_and_checkpoint(storage, epoch, raw_ckpt, proof_epoch_sealed, &txs_info)
            .map_err(|e| StdError::generic_err(format!("failed to handle Babylon epoch from Babylon: {e}")))?;
    } else {
    ...
    if let Some(cz_header) = btc_ts.header.as_ref() {
        ...
        let proof_tx_in_block = proof.proof_tx_in_block.as_ref().ok_or(StdError::generic_err("empty proof_tx_in_block"))?;
        let proof_header_in_epoch = proof.proof_header_in_epoch.as_ref().ok_or(StdError::generic_err("empty proof_header_in_epoch"))?;
        cz_header_chain::handle_cz_header(storage, &cz_header, &epoch, &proof_tx_in_block, &proof_header_in_epoch)
            .map_err(|e| StdError::generic_err(format!("failed to handle CZ header from Babylon: {e}")))?;
        ...
        let ts_babylon_header = cz_header.babylon_header.as_ref().ok_or(StdError::generic_err("empty babylon_header"))?;
        let ts_time = ts_babylon_header.time.as_ref().ok_or(StdError::generic_err("empty time"))?.seconds; // TODO: use time in IndexedHeader
