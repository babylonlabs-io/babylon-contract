...
pub mod tests {
    ...
    fn test_update_contract_addresses() {
        ...
        let update_msg = ExecuteMsg::UpdateContractAddresses {
            btc_light_client: "new_btc_light_client".to_string(),
            finality: "new_finality".to_string(),
        };
        let err = execute(
            deps.as_mut(),
            mock_env(),
        ...
        assert_eq!(config.finality, Addr::unchecked("new_finality"));
        assert_eq!(
            config.btc_light_client,
            Addr::unchecked("new_btc_light_client")
        );

        // Update with babylon
        let babylon_info = message_info(&deps.api.addr_make(CREATOR), &[]);
        let update_msg = ExecuteMsg::UpdateContractAddresses {
            btc_light_client: "another_btc_light_client".to_string(),
            finality: "another_finality".to_string(),
        };
        ...
        let config = queries::config(deps.as_ref()).unwrap();
        assert_eq!(config.finality, Addr::unchecked("another_finality"));
        assert_eq!(
            config.btc_light_client,
            Addr::unchecked("another_btc_light_client")
        );
    }
}
